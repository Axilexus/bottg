...
def show_schedule(_, message: Message):
    ...
    for day in days:
        ...
        schedule_message += "\n\n"
    schedule_message += "Открываю сбор на поддержку бота. Оплата: 7 рублей с каждого. Кидаем на номер +79303838540. банк почти любой"
...
def show_homework(_, message: Message):
    ...
    # Отправляем сообщение с расписанием
    message.reply_text(schedule_message)
    message.reply_sticker(sticker='CAACAgIAAxkBAAJVWGUvfS-sBko3hCnSiJYxd1_-FD8-AAIIGwACMTfoSg9FFtK9MbUwMAQ')
    conn.close()
